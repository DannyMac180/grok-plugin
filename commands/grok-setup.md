...
3. If the user also wants Grok in **Codex**: they need the CLI installed globally (`npm install -g grok-bridge` or `npm install -g .` from a checkout of this repo), then `grok-bridge install codex` adds the same MCP server to `~/.codex/config.toml`.
